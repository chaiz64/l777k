...
def clean_name(input_text):
    # FFmpeg interprets a literal '%' in an output filename as the start of a
    # printf-style format specifier (used internally for segment numbering).
    # If it isn't followed by a valid specifier, FFmpeg mis-parses the path
    # and silently fails to write the file. Swap it for a visually similar
    # character ('%' -> '﹪', SMALL PERCENT SIGN U+FE6A) so the name still
    # reads the same but FFmpeg no longer treats it specially.
    cleaned_name = input_text.strip().replace('%', '﹪')
    cleaned_name = re.sub(rstr, "_", cleaned_name).strip('_')
